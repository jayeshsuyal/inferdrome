#!/usr/bin/env python3
"""Verify the reviewed A100 SXM4 archive and open its read-only dashboard."""
...
try:
    ...
    import scripts.review_qwen3_a100_sxm4_evidence as publication
    ...
    import review_qwen3_a100_sxm4_evidence as publication  # type: ignore[no-redef]


class A100DashboardError(RuntimeError):
    """The reviewed A100 dashboard could not be opened safely."""
...
def verify_dashboard_source(
    ...
    """Require exact archive bytes, receipts, and committed anchors."""

    try:
        outputs = publication.render_outputs(archive, capture_record)
    except (
        OSError,
        publication.A100EvidenceError,
        publication.PublicationReviewError,
        capture.CaptureError,
        qwen3_capture.Qwen3CaptureError,
    ) as error:
        raise A100DashboardError(str(error)) from None
        ...
        json.dumps(
            {
                ...
                "dashboard_source": "VERIFIED_REVIEWED_QWEN3_A100_SXM4_CAPTURE",
...
def _launch(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix="inferdrome-qwen3-a100-sxm4-evidence-dashboard-"
    ) as temporary:
        ...
        try:
            ...
            if verification["run"]["run_id"] != publication.RUN_ID:
                raise A100DashboardError("verified dashboard run identity drifted")
            ...
            print(
                "Opening reviewed genuine-GPU evidence. This is a bounded A100 "
                "SXM4 observation, not an acceptance verdict or cross-GPU comparison.",
                flush=True,
            )
            return subprocess.run(command, check=False).returncode
        finally:
            # The archive preserves sealed workspace modes.  Relax only the
            # disposable temporary extraction so macOS/Linux cleanup succeeds;
            # the dashboard still reads the untouched bundle before cleanup.
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Verify the reviewed Qwen3-8B A100 SXM4 capture and open the "
            "local dashboard"
        )
    )
    parser.add_argument(
...
def main() -> int:
    ...
    except A100DashboardError as error:
        print(f"A100 SXM4 evidence dashboard: {error}", file=sys.stderr)
        return 1
