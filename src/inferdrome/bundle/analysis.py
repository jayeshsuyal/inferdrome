...
def verify_bundle_matches_workspace(
    ...
    if workspace.current_state().state is not RunState.COMPLETE:
        raise VerificationError("bundle workspace is not COMPLETE")
    selected = analysis or recalculate_bundle(workspace.path / "bundle")
    ...
    for workspace_path, role in comparisons:
        workspace_bytes = workspace.read_frozen_input(workspace_path)
    ...
    workload_bytes = workspace.read_frozen_input("inputs/workload.source.jsonl")
