...
| --- | --- |
| Missing, forged, stale, or profile-mismatched watchdog activation | v0.2 has no reviewed private two-A100 activation worker, so the create-capable factory and CLI `execute` command fail closed before SDK/client construction, journaled intent, or insert. A future live bridge must re-read a canonical durable receipt bound to this exact profile before either factory construction or insert. |
| Unbounded controller intent | `CREATE_INTENT_DURABLE` is only a fsynced local intent and recovery record, never a watchdog or provider backstop. v0.2 creates no private-campaign VM. A future live bridge must arm a separate provider-capable watchdog before create; observed Compute `maxRunDuration` plus `DELETE` remains a provider backstop, not invoice enforcement. |
...
| IAP/guest-control-plane drift | The approved controller principal is the explicit impersonated identity for every Compute client and IAP argv; the named IAP firewall identity/CIDR/tag/three-port policy, non-default zero-scope guest service account, blocked inherited project SSH keys, disabled OS Login, and private NIC must all be read back. Any mismatch blocks admission. Ambient `gcloud auth list` state is not an authorization signal. |
| Other effective VPC firewall rule | Docker-published ports 8000–8002 are private-interface listeners. The controller verifies only the named IAP rule, so the live operator must independently establish that no other effective rule admits a non-IAP source to the approval-bound tag/ports. This remains an explicit operator precondition, not a platform-wide firewall claim. |
...
| Evidence-root replacement, archive tamper, or post-seal crash | The runner seals a fixed immutable package inventory with create-no-replace semantics. The controller accepts only a bounded IAP archive with no redirect, duplicate member, link, unsafe mode, oversized member, or malformed manifest, binds its transfer/config/workload and both endpoint origins to the exact admitted handoff, then re-verifies the package under its held evidence root. Redacted readiness and sealed-artifact receipts are durably published and journaled; recovery reopens and verifies artifacts rather than using in-memory state or rerunning a sealed workload. An evidence-root fault is retained in the recovery result after—not before—exact cleanup. |
