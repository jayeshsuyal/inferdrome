...
def test_determinism_and_stable_zone_machine_tie_break() -> None:
    ...
    payload["zones"] = list(reversed(payload["zones"]))
    payload["machine_types"] = list(reversed(payload["machine_types"]))
    ...
    assert first.selection.selected_provider.zone == "us-central1-a"
...
@pytest.mark.parametrize(
    "mutation",
    [
        lambda value: [zone.update(status="unavailable") for zone in value["zones"]],
        lambda value: value["machine_types"][0]["accelerators"][0].update(
            status="unknown"
        ),
        lambda value: value["machine_types"][0].update(cpu_cores=1),
        lambda value: value["machine_types"][0]["accelerators"][0].update(
            accelerator_model="NVIDIA H100 PCIe"
        ),
        lambda value: value["machine_types"][0].update(architecture="arm64"),
    ],
)
...
def test_plan_cross_input_substitution_and_model_copy_are_rejected(
    ...
    altered_inventory_value["zones"][0]["status"] = "unavailable"
